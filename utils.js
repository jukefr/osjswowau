...
const firstStart = (config) => {
    ...
    if (
        (config.get("realpath") === "/home/user/Games/world-of-warcraft/drive_c/Program Files (x86)/World of Warcraft/_retail_/Interface/AddOns")
    ) {
        log.info("First run detected.");
        log.info(`Please edit ${chalk.yellow(config.path)} to match your needs.`);
        (process.platform === 'win32') && log.info(`Make sure to use double backslashes ${chalk.yellow('\\\\')} to escape the ${chalk.yellow('realpath')} (AddOns folder) variable.`);
        (process.platform === 'win32') && log.info(`ie. ${chalk.yellow('"C:\\\\Program Files\\\\..."')}`);
        log.info()
        log.info(`If this is a mistake you can set ${chalk.yellow('"firstStart": false')} in the configuration file.`);
        process.exit(1);
    }
}
